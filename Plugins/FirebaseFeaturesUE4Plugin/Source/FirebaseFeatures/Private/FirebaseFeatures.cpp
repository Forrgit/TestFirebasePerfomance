...
static bool CopyGoogleServicesPlistToLocation(const FString& TargetLocation)
{
	...
	return IPlatformFile::GetPlatformPhysical().CopyFile(*TargetLocation, *(FPaths::ProjectDir() / TEXT("Config/Firebase/GoogleService-Info.plist")));
}
#endif
...
void FFirebaseFeaturesModule::StartupModule()
{
...

	{
		...
		const FString iOSServicesLocation     = FPaths::ProjectDir() / TEXT("Config/Firebase/GoogleService-Info.plist");
		const FString AndroidServicesLocation = FPaths::ProjectDir() / TEXT("Config/Firebase/google-services.json");
...
void FFirebaseFeaturesModule::ShutdownModule()
{
	...
	{
#if WITH_FIREBASE_AUTH
		if (firebase::App::GetInstance() && firebase::auth::Auth::GetAuth(firebase::App::GetInstance()))
		{
			delete firebase::auth::Auth::GetAuth(firebase::App::GetInstance());
		}
#endif

#if WITH_FIREBASE_FIRESTORE
		if (firebase::App::GetInstance() && firebase::firestore::Firestore::GetInstance(firebase::App::GetInstance()))
		{
			delete firebase::firestore::Firestore::GetInstance(firebase::App::GetInstance());
		}
#endif

#if WITH_FIREBASE_DATABASE
		if (firebase::App::GetInstance() && firebase::database::Database::GetInstance(firebase::App::GetInstance()))
		{
			delete firebase::database::Database::GetInstance(firebase::App::GetInstance());
		}
#endif

#if WITH_FIREBASE_STORAGE
		if (firebase::App::GetInstance() && firebase::storage::Storage::GetInstance(firebase::App::GetInstance()))
		{
			delete firebase::storage::Storage::GetInstance(firebase::App::GetInstance());
		}
#endif

#if WITH_FIREBASE_FUNCTIONS
		if (firebase::App::GetInstance() && firebase::functions::Functions::GetInstance(firebase::App::GetInstance()))
		{
			delete firebase::functions::Functions::GetInstance(firebase::App::GetInstance());
		}
#endif
...
#		endif
#	else
		delete firebase::remote_config::RemoteConfig::GetInstance(firebase::App::GetInstance());
#	endif
...
void FFirebaseFeaturesModule::InitializeFirebaseComponents()
{
	...
	InitFirebaseModule(Crashlytics);
...
void FFirebaseFeaturesModule::InitCrashlytics()
{
#if WITH_FIREBASE_CRASHLYTICS
	UCrashlyticsLibrary::SetCrashlyticsCollectionEnabled(UFirebaseConfig::Get()->bCrashlyticsCollectionEnabled);

	CrashlyticsProxy = NewObject<UCrashlyticsProxy>(GetTransientPackage());
	CrashlyticsProxy->AddToRoot();
	CrashlyticsProxy->InitializeCrashlytics();
#endif // WITH_FIREBASE_CRASHLYTICS
}
...
{
	return FPaths::ProjectDir() / TEXT("Config/Firebase/google-services.json");
}
