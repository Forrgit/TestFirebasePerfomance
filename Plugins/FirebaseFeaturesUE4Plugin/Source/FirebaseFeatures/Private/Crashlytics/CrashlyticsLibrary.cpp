...
static jmethodID GetGameActivityMethodID(JNIEnv* const Env, const char* MethodName, const char* MethodSignature)
{
	...
	if (Env->ExceptionOccurred())
	{
		...
		UE_LOG(LogFirebaseCrashlytics, Error, TEXT("Failed to get method %s with signature %s."), UTF8_TO_TCHAR(MethodName), UTF8_TO_TCHAR(MethodSignature));
...
#define CALL_FIR_CRASHLYTICS_JAVA_METHOD(MethodName, MethodSignature, ReturnType, DefaultReturn, ...)                                                                                                             \
	...
	{                                                                                                                                                                                                             \
		...
		if (jMethod == 0)                                                                                                                                                                                         \
		{                                                                                                                                                                                                         \
			UE_LOG(LogFirebaseCrashlytics, Error, TEXT("Failed to get method %s with signature %s. "), TEXT("Did you forget to enable Crashlytics?"), UTF8_TO_TCHAR(MethodName), UTF8_TO_TCHAR(MethodSignature)); \
                                                                                                                                                                                                                  \
...
void UCrashlyticsLibrary::RecordError(const FString& Domain, const FString& Message, const int64 Code)
{
	...
	CALL_FIR_CRASHLYTICS_JAVA_METHOD("FIR_CL_RecordException", "(Ljava/lang/String;)V", Void, void(), *FJavaHelper::ToJavaString(Env, ThrowableMessage));
#endif
#endif
}
...
#if PLATFORM_IOS
	[[FIRCrashlytics crashlytics] setCustomValue:FString::Printf(TEXT("%f"), Value).GetNSString()
...
#if PLATFORM_IOS
	[[FIRCrashlytics crashlytics] setCustomValue:FString::Printf(TEXT("%lld"), Value).GetNSString()
	                                      forKey:Key.GetNSString()];
#elif PLATFORM_ANDROID
...
#if PLATFORM_IOS
	[[FIRCrashlytics crashlytics] setCustomValue:FString::Printf(TEXT("%d"), Value).GetNSString()
	                                      forKey:Key.GetNSString()];
#elif PLATFORM_ANDROID
...
#if PLATFORM_IOS
	[[FIRCrashlytics crashlytics] setCustomValue:FString(Value ? TEXT("True") : TEXT("False")).GetNSString()
	                                      forKey:Key.GetNSString()];
#elif PLATFORM_ANDROID
...
#if PLATFORM_IOS
	[[FIRCrashlytics crashlytics] setCustomValue:Value.GetNSString()
	                                      forKey:Key.GetNSString()];
#elif PLATFORM_ANDROID
...
void UCrashlyticsLibrary::CheckForUnsentReports()
{
#if WITH_FIREBASE_CRASHLYTICS
	UE_LOG(LogFirebaseCrashlytics, Log, TEXT("Check For Unsent Reports."));

#if PLATFORM_IOS
	[[FIRCrashlytics crashlytics] checkForUnsentReportsWithCompletion:^(BOOL hasUnsentReports) {
	  if (auto proxy = GetCrashlyticsProxy())
	  {
		  proxy->bUnsentReportsWasChecked = true;
		  proxy->bHasUnsentReports = hasUnsentReports;
	  }
	}];
#elif PLATFORM_ANDROID
	CALL_FIR_CRASHLYTICS_JAVA_METHOD("FIR_CL_CheckForUnsentReports", "()V", Void, void());
#endif
#endif
}

bool UCrashlyticsLibrary::HasUnsentReports()
{
#if WITH_FIREBASE_CRASHLYTICS
#if PLATFORM_IOS
	if (auto proxy = GetCrashlyticsProxy())
	{
		return proxy->bHasUnsentReports;
	}
	return false;
#elif PLATFORM_ANDROID
	CALL_FIR_CRASHLYTICS_JAVA_METHOD("FIR_CL_HasUnsentReports", "()Z", Boolean, false);
#else
	return false;
#endif
#else
...
bool UCrashlyticsLibrary::IsUnsentReportsWasChecked()
{
#if WITH_FIREBASE_CRASHLYTICS
#if PLATFORM_IOS
	if (auto proxy = GetCrashlyticsProxy())
	{
		return proxy->bUnsentReportsWasChecked;
	}
	return false;
#elif PLATFORM_ANDROID
	CALL_FIR_CRASHLYTICS_JAVA_METHOD("FIR_CL_IsUnsentReportsWasChecked", "()Z", Boolean, false);
#else
	return false;
#endif
#else
...
UCrashlyticsProxy* UCrashlyticsLibrary::GetCrashlyticsProxy()
{
	static auto moduleName = TEXT("FirebaseFeatures");
	if (FModuleManager::Get().IsModuleLoaded(moduleName))
	{
		const auto& firebaseModule = FModuleManager::LoadModuleChecked<FFirebaseFeaturesModule>(moduleName);
		return firebaseModule.CrashlyticsProxy.Get();
	}
	return nullptr;
}
