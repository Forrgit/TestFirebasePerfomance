...
public class FirebaseFeatures : ModuleRules
{
	...
	{ 
		...
		else if (Target.Platform == UnrealTargetPlatform.IOS)
		{
            ...
            { 
				...
				AddFrameworkChecked("GoogleToolboxForMac");
			...
			if (System.Array.IndexOf(FirebaseFrameworks, "firebase_instance_id") >= 0)
			{
				AddFirebaseFramework("firebase_instance_id");
			...
			if (bEnableStorage && System.Array.IndexOf(FirebaseFrameworks, "firebase_storage") >= 0)
			{
				...
				AddFirebaseFramework("FirebaseStorage");
				AddFrameworkChecked("GTMSessionFetcher");
			}
			...
			if (System.Array.IndexOf(FirebaseFrameworks, "firebase_auth") >= 0)
			{
			...
			if (bEnableAnalytics && System.Array.IndexOf(FirebaseFrameworks, "firebase_analytics") >= 0)
			{
				...
				AddFrameworkChecked("FIRAnalyticsConnector");
			...
			// Required plist file.
			if (!File.Exists(Path.Combine(ProjectDirectory, "Config/Firebase/GoogleService-Info.plist")) &&
		...
		else if (Target.Platform == UnrealTargetPlatform.Linux)
		{
			...
			if (bLinuxLinkPICLibraries)
			{
				...
				{
					if (!Dependency.EndsWith(".so.0") && !Dependency.EndsWith(".so.7"))
					{
	...
	private void LinkFirebaseLibraries()
	{
		...
		if (System.Array.IndexOf(FirebaseLibraries, "firebase_auth") >= 0)
        {
    ...
    {
		return "$(ProjectDir)/Config/Firebase/";
	}
    ...
    {
		AddFrameworkChecked(FrameworkName);
	...
	private void CheckEnvironment(ReadOnlyTargetRules Target)
    {
		...
		if (bIsPluginInEngineDir)
		{
			// Change true to false if you don't want this message to appear anymore.
			if (true)
			{
		...
		{
			System.Console.WriteLine("Firebase Features is being included in a build as a Project plugin. Linkage will follow " +
